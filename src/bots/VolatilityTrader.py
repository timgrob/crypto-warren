...
# from strategies.trading_strategies_old import TradingStrategy


# class VolatilityTrader(TradingBot):
#     def __init__(
#         self, exchange: ccxt.Exchange, trading_strategy: TradingStrategy
#     ) -> None:
...
#             for tr in trades:
#                 trade = Trade(
#                     timestamp=tr.timestamp, symbol=tr.symbol, qty=tr.qty, price=tr.price
#                 )
...
#         logging.basicConfig(
#             filename=os.path.join(logfile_path, "logfile.log"), level=logging.INFO
#         )
...
#             invest_cash_amount = total_cash / (
#                 self.max_number_investment - self.trades.qsize()
#             )
...
#             passed_24h = (
#                 True
#                 if (datetime.now() - timedelta(hours=24)) > last_buy_trade.timestamp
#                 else False
#             )

#             if (
#                 self.trading_strategy.sell_signal(token_data)
#                 and not self.trades.empty()
#                 and token_data["bid"] > last_buy_trade.price
#             ):
#                 last_buy_trade = self.trades.get()
#                 sell_order = self.exchange.create_limit_sell_order(
#                     token_data["symbol"], last_buy_trade.qty, token_data["bid"]
#                 )
#                 with Session() as session:
#                     with session.begin():
